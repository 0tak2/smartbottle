...
import TdsDataTable from './TdsDataTable.js'

const tdsBaseUrl = '/api/tds'
...
export default {
    ...
    setup() {
        ...
        async function handleExport() {
            ...
            if (query.dateOption.active) {
                ...
                requestUrl = `${tdsBaseUrl}?size=10&page=${query.page}&start_time=${start}&end_time=${end}`
            } else {
                requestUrl = `${tdsBaseUrl}?size=10&page=${query.page}`
            }
        ...
        async function fetchTdsData() {
            ...
            try {
                ...
                fetchedData.tds = response.data.result
            } catch (error) {
                console.error(error)
            }

            if (fetchedData.tds.length === 0) {
    ...
    template: `
        ...
        <div v-if="!isLoading" class="full-width">
            <tds-data-table v-bind:tdsData="fetchedData.tds"></tds-data-table>
